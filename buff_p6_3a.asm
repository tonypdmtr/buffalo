...
; Rev P1    2/03/00 - First Modified to operate with CPU_1A1 & CPU_1B
;                   - Revision numbering system changed to indicate target
;                       change.
; Rev P2   19/08/00 - Modified TMSK2 initalisation to 00 for Trace command
; Rev P3   29/09/02 - First Modified to operate with CPU_3A series boards
; Rev P4   22/02/03 - Modified to disable restart based on condition of
;                       port-E bit-0
; Rev P5   17/07/03 - Modified to prevent read of $1060-$19FF
; Rev P6   29/11/04 - Disable writing to DFLOP (address $4000)
;
;
;****************************************************
...
; Modified      EQU     Paul Bealing for CPU_3A*

RAMBS               equ       $0000               ; start of ram
REGBS               equ       $1000               ; start of registers
ROMBS               equ       $8000               ; start of rom
...
CSSTRH              equ       REGBS+$5C           ; CS CYCLE STRETCH
CSCTL               equ       REGBS+$5D           ; CS CONTROL
CSGADR              equ       REGBS+$5E           ; GENERAL PURPOSE CS (RAM)
CSGSIZ              equ       REGBS+$5F           ; GENERAL PURPOSE CS SIZE
...
; INITIAL CPU SETUP FOR LOADER
                    ...
                    org       DSTREE+$180         ; special variable area

BOOT1               ldx       #$1000              ; register base address

                    ldaa      #%10010001          ; adpu, irqe, dly, cop = 65mS
                    staa      OPTION
                    ldaa      #%00000000          ; TMSK2 REG. DATA
                    staa      TMSK2
                    ldaa      #$00
                    staa      BPROT               ; make CONFIG & EEPROM writable

                    lds       #$3FF               ; NORMALLY 03FFH

                    ldaa      #%00000000
                    staa      [CSSTRH,X           ; CS cycle stretch
                    ldaa      #%10000101
                    staa      [CSCTL,X            ; enable program CS for 32K and bank
                    ldaa      #%00000000
                    staa      [CSGADR,X           ; RAM starts at address 0000H
                    ldaa      #%10000001
                    staa      [CSGSIZ,X           ; RAM block size is 32K

                    ldaa      #%11111111          ; enable SRAM (CE2) and select banks
                    staa      BANKSR              ; select banks (Flash and RAM)
                    staa      BANKSS              ; make working copy

                    jmp       BUFFALO             ; point to APPLICATION in FLASH

;*************
; INTERRUPT VECTORS

                    org       DENDEE-$29          ; TOP OF ROM
INTVECT
...
OUT1BYT             psha
; pmb insert start
                    cpx       #$1060              ; check lower end
                    blo       OUT1B1              ; OK because address is < $1060
                    cpx       #$17FF              ; check top end
                    bhi       OUT1B1              ; OK because address is > $17FF
                    ldaa      #$00                ; otherwise force read as $00
                    bra       OUT1B2              ; continue

; pmb insert end
OUT1B1              ldaa      0,X                 ; get data in a
OUT1B2              psha                          ; save copy
...
MSG1                fcc       'BUFFALO 3.4,P5 (ext) - Bit User Fast Friendly Aid to Logical Operation'
                    fcb       EOT
MSG2                fcc       'What?'
                    fcb       EOT
MSG3                fcc       'Too Long'
...
; pmb insert start
                    cpx       #$1060              ; check lower end
                    blo       DUMP6               ; OK because address is < $1060
                    cpx       #$17FF              ; check top end
                    bhi       DUMP6               ; OK because address is > $17FF
                    ldaa      #$00                ; otherwise force read as $00
                    bra       DUMP7               ; continue

; pmb insert end
DUMP6               ldaa      ,X                  ; ascii value loop
DUMP7               cmpa      #$20
...
; pmb insert start
                    cpx       #$1060              ; check lower end
                    blo       FILL3               ; OK because address is < $1060
                    cpx       #$17FF              ; check top end
                    bhi       FILL3               ; OK because address is > $17FF
                    bra       FILL4               ; continue, skip the write

; pmb insert end
FILL3               jsr       WRITE               ; write the data to x
                    cmpa      0,X
